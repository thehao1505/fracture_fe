...
| --------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| **Anonymous**                     | none                                                | `POST /auth/register`, `POST /auth/login`, `GET /profile/:username`, `POST /profile/:username/blocks/:id/click`, `GET /health` |
...
> user can operate on _any_ user id. Only `/me/*` is scoped to the token owner
...
- **API prefix / versioning:** all business routes are under **`/api/v1`** (`cmd/api/main.go`). `/health` is **not** under the prefix.
- **API docs:** there is no in-app API documentation endpoint (Swagger has been removed). This file is the API contract.
...
`RegisterRoutes`), which is authoritative.
...
- Errors: `400` invalid body / bad username / bad appearance; `409` `{"error":"resource already exists"}` — the user **already has a profile** _or_ the username is taken (`profile_usecase.go:246,257`).
...
Mounted at `/api/v1/profile` (`main.go`).
...
- `409` on register/create-user → "email already in use"; on create/update profile → "username taken" _or_ "you already have a profile" (same message — disambiguate by context).
...
2. **~~Public profile path discrepancy.~~** _(Resolved.)_ The public profile routes are `/api/v1/profile/:username` and `/api/v1/profile/:username/blocks/:id/click`. The old Swagger `/p/{username}` annotations have been removed along with Swagger itself.
